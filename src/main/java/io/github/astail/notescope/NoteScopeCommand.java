...
/** /notescope … 自分のアクションバー表示を ON/OFF（引数なしはトグル）するコマンド。 */
public final class NoteScopeCommand implements TabExecutor {

    private static final List<String> SUBCOMMANDS = List.of("on", "off", "status");

    private final NoteScopePlugin plugin;

    public NoteScopeCommand(@NotNull NoteScopePlugin plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command,
        ...
        // notescope.use の有無は plugin.yml の commands.notescope.permission により Bukkit が事前チェック済み。
        ...
        if (args.length != 1) {
            player.sendMessage("§e使い方: /notescope [on|off|status]");
            return true;
        }
        ...
        switch (args[0].toLowerCase(Locale.ROOT)) {
            ...
            case "status" -> player.sendMessage(current
                    ? "§a音階表示は ON です。音符ブロックを見ると表示されます。"
                    : "§7音階表示は OFF です。/notescope on で有効化できます。");
            default -> player.sendMessage("§e使い方: /notescope [on|off|status]");
        }
        return true;
    }
    ...
    private void setAndReport(Player player, boolean enabled) {
        ...
        player.sendMessage(enabled
                ? "§a音階表示を ON にしました。音符ブロックを見るとアクションバーに表示されます。"
                : "§7音階表示を OFF にしました。");
    }
